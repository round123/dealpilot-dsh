...
export const APPROVAL_SCHEMA = 'dealpilot.approval/v2';
...
function jsonValue(value, stack = new Set()) {
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw new Error('Approval payload must contain finite JSON numbers');
        return value;
    }
    if (value === undefined)
    ...
    if (Buffer.isBuffer(value))
        return { $buffer_base64: value.toString('base64') };
    if (typeof value !== 'object')
        return value;
    if (stack.has(value))
        throw new Error('Approval payload must not contain cycles');
    stack.add(value);
    try {
...
function normalizeWorkspace(workspacePath) {
    if (!path.isAbsolute(workspacePath))
        throw new Error('Approval workspace must be an absolute path');
...
export function workspaceFingerprint(workspacePath) {
...
function readRecord(filePath) {
    try {
        ...
        if (value?.schema !== APPROVAL_SCHEMA || typeof value?.approval_id !== 'string')
            return undefined;
        return value;
    }
    catch {
...
function readRecoverableRecord(filePath) {
...
function withStoreLock(root, callback) {
    ...
    while (descriptor === undefined) {
        try {
            ...
            if (Date.now() - started >= LOCK_WAIT_MS)
                throw new Error('Approval store is busy; retry the operation');
            wait(10);
        }
    }
    try {
...
function recordsIn(root) {
    ...
    for (const name of names) {
        if (/^apr_[a-f0-9-]+\.json$/iu.test(name))
            ids.add(name);
        else if (/^apr_[a-f0-9-]+\.json\.before\.bak$/iu.test(name))
            ids.add(name.slice(0, -'.before.bak'.length));
    }
...
function expirePending(root, now = Date.now()) {
    for (const item of recordsIn(root)) {
        if (item.record.status !== 'pending' || Date.parse(item.record.expires_at) > now)
            continue;
...
function validateBinding(record, input, identity) {
...
export function validateApproval(token, input) {
    if (!token?.startsWith('dpa_'))
        throw new Error('批准令牌无效');
    ...
    return withStoreLock(root, () => {
        ...
        if (!item)
            throw new Error('批准令牌不存在或不属于当前 Workspace');
        if (item.record.status !== 'pending')
            throw new Error(`批准记录当前状态为 ${item.record.status}，不能使用`);
        if (Date.parse(item.record.expires_at) <= now)
            throw new Error('批准记录已过期，请重新审阅');
 ...
 * changing its status. */
export function validateApprovalRecord(record, input) {
    if (!record || record.schema !== APPROVAL_SCHEMA || !record.approval_id)
        throw new Error('批准记录无效');
    const identity = workspaceIdentity(input);
    const root = approvalRoot(identity.workspacePath);
    return withStoreLock(root, () => {
        const filePath = approvalPath(root, record.approval_id);
        const current = readRecoverableRecord(filePath);
        if (!current || current.token_hash !== record.token_hash)
            throw new Error('批准记录不存在或已被替换');
        const now = Date.now();
        expirePending(root, now);
        ...
        if (fresh.status !== 'pending')
            throw new Error(`批准记录当前状态为 ${fresh.status}，不能使用`);
        if (Date.parse(fresh.expires_at) <= now)
            throw new Error('批准记录已过期，请重新审阅');
...
export function createApproval(input) {
    ...
    if (!input.sessionId?.trim())
        throw new Error('Approval session is required');
    if (!input.schemaVersion?.trim())
        throw new Error('Approval schema version is required');
    if (input.changeSetHash !== undefined && !/^[a-f0-9]{64}$/iu.test(input.changeSetHash))
        throw new Error('Approval change-set hash must be a SHA-256 digest');
    if (input.changeSetId !== undefined && !input.changeSetId.trim())
        throw new Error('Approval change-set id cannot be empty');
    if (input.interpretationId !== undefined && !input.interpretationId.trim())
        throw new Error('Approval interpretation id cannot be empty');
    ...
    const record = {
        ...
        resolutions: (jsonValue(input.resolutions || {}) || {}),
        payload,
...
export function consumeApproval(token, input) {
    if (!token?.startsWith('dpa_'))
        throw new Error('批准令牌无效');
    const identity = workspaceIdentity(input);
    const root = approvalRoot(identity.workspacePath);
    ...
    return withStoreLock(root, () => {
        ...
        if (!item)
            throw new Error('批准令牌不存在或不属于当前 Workspace');
        if (item.record.status !== 'pending')
            throw new Error(`批准记录当前状态为 ${item.record.status}，不能再次使用`);
        if (Date.parse(item.record.expires_at) <= now)
            throw new Error('批准记录已过期，请重新审阅');
        validateBinding(item.record, input, identity);
...
export function consumeApprovalRecord(record, input) {
    if (!record || record.schema !== APPROVAL_SCHEMA || !record.approval_id)
        throw new Error('批准记录无效');
    const identity = workspaceIdentity(input);
    const root = approvalRoot(identity.workspacePath);
    return withStoreLock(root, () => {
        ...
        if (!fresh)
            throw new Error('批准记录不存在或已被替换');
        if (fresh.status !== 'pending')
            throw new Error(`批准记录当前状态为 ${fresh.status}，不能再次使用`);
        if (Date.parse(fresh.expires_at) <= now)
            throw new Error('批准记录已过期，请重新审阅');
        validateBinding(fresh, input, identity);
...
export function readApproval(workspacePath, approvalId) {
...
export function listApprovals(workspacePath, status) {
    const identity = workspaceIdentity({ workspacePath });
    const root = approvalRoot(identity.workspacePath);
    return withStoreLock(root, () => {
        expirePending(root);
...
export function revokeApproval(workspacePath, approvalId, actor = 'user', reason = 'revoked') {
    const identity = workspaceIdentity({ workspacePath });
    const root = approvalRoot(identity.workspacePath);
    return withStoreLock(root, () => {
        ...
        const record = readRecoverableRecord(filePath);
        if (!record)
            throw new Error(`批准记录不存在：${approvalId}`);
...
export function revokePendingApprovals(workspacePath, actor = 'system', reason = 'session cleared') {
    const identity = workspaceIdentity({ workspacePath });
    const root = approvalRoot(identity.workspacePath);
    return withStoreLock(root, () => {
        let count = 0;
