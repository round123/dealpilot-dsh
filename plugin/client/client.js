window.__ModuleLoader__.load({ id: "dealpilot-dsh", factory: (require) => {
...
function mountDealPilot(runtime) {
    ...
    const renderCustomerDetail = async (item, detail) => {
        const version = ++detailVersion;
        const customerRef = String(item.ref || '');
        const deals = (snapshot?.deals || []).filter((deal) => deal.customer_ref === customerRef || deal.customer_name === item.title);
        const dealRefs = new Set(deals.map((deal) => deal.ref));
        const timeline = (snapshot?.activity || [])
            .filter((event) => event.customer_ref === customerRef || dealRefs.has(event.deal_ref))
            .map((event) => ({ date: event.occurred_at, label: event.event_type || '业务活动', text: event.summary || event.channel || '', source: event.source_ref }));
        for (const deal of deals) {
            if (deal.updated_at)
                timeline.push({ date: deal.updated_at, label: '交易更新', text: deal.title, source: deal.ref });
            for (const action of deal.actions || [])
                if (action.updated_at || action.due_at)
                    timeline.push({ date: action.updated_at || action.due_at, label: '跟进任务', text: `${action.title}${action.status ? ` · ${action.status}` : ''}`, source: action.ref });
        }
        if (item.updated_at)
            timeline.push({ date: item.updated_at, label: '客户资料更新', text: item.title, source: customerRef });
        const uniqueTimeline = timeline.filter((entry, index, rows) => rows.findIndex((candidate) => candidate.date === entry.date && candidate.label === entry.label && candidate.text === entry.text) === index)
            .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
        const metadataRows = Object.entries(item.extra_metadata || {});
        const contactRows = (item.contacts || []).map((contact) => `<div class="dealpilot-contact-row"><strong>${escapeHtml(contact.title || '未命名联系人')}</strong><span>${escapeHtml([contact.role, contact.email, contact.phone].filter(Boolean).join(' · ') || '暂无联系方式')}</span></div>`).join('');
        const dealRows = deals.map((deal, index) => `<button class="dealpilot-related-row" data-related-deal="${index}" type="button"><strong>${escapeHtml(deal.title)}</strong><span>${escapeHtml([deal.funnel_stage, deal.risk_level, deal.status].filter(Boolean).join(' · ') || '交易')}</span></button>`).join('');
      ...
      <section class="dealpilot-detail-section"><h4>时间线 <span>${uniqueTimeline.length}</span></h4><div class="dealpilot-timeline">${uniqueTimeline.length ? uniqueTimeline.map((entry) => `<div class="dealpilot-timeline-row"><time>${escapeHtml(formatDate(entry.date, true))}</time><span><strong>${escapeHtml(entry.label)}</strong><small>${escapeHtml(entry.text || '已记录')}</small></span></div>`).join('') : '<p class="dealpilot-muted">暂无可见活动</p>'}</div></section>
      ...
      <div class="dealpilot-detail-source" data-source-state>正在读取完整客户档案...</div>`;
        ...
        detail.querySelectorAll('[data-related-deal]').forEach((button) => button.addEventListener('click', () => { selectedItem = deals[Number(button.dataset.relatedDeal)]; renderBoard('deals'); }));
        try {
            if (!memoryCache.has(customerRef))
                memoryCache.set(customerRef, await api(`/api/dealpilot/memory?workspaceId=${encodeURIComponent(selectedId)}&ref=${encodeURIComponent(customerRef)}`));
            if (version !== detailVersion)
                return;
            const document = memoryCache.get(customerRef);
            const source = detail.querySelector('[data-source-state]');
            if (source)
                source.innerHTML = `<strong>完整档案</strong><pre class="dealpilot-memory">${escapeHtml(String(document.content || '').trim() || '正文为空')}</pre>`;
        }
        catch (err) {
            const source = detail.querySelector('[data-source-state]');
            if (source) {
    ...
    const renderDetail = (item, view) => {
        ...
        if (view === 'customers') {
            void renderCustomerDetail(item, detail);
            return;
        }
        const fields = [];
        if (view === 'customers')
    ...
    const renderBoard = (view) => {
        ...
        panel.querySelectorAll('[data-sidebar-view]').forEach((button) => button.classList.toggle('active', button.dataset.sidebarView === view));
        const content = workbench.querySelector('[data-board-content]');
        if (!snapshot) {
