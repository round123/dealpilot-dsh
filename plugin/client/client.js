...
function mountDealPilot(runtime) {
    ...
    const droppedImportFile = (event) => Array.from(event.dataTransfer?.files || []).find(file => importExtensions.test(file.name));
    document.addEventListener('dragover', event => {
        if (!droppedImportFile(event))
            return;
    ...
    const load = async () => {
        try {
            ...
            select.innerHTML = '<option value="">选择工作区</option>' + (data.workspaces || []).map((w) => `<option value="${escapeHtml(w.id)}"${w.status === 'archived' ? ' disabled' : ''}>${escapeHtml(w.name)}${w.status === 'archived' ? '（已归档）' : ''}</option>`).join('');
...
function boardSubtitle(view, snapshot) {
    ...
    const copy = { today: '聚焦需要推进的销售事项', customers: '管理客户关系、市场和优先级', deals: '掌握阶段、风险和下一步行动', actions: '安排、推进和完成跟进', funnel: '查看活跃交易的阶段分布', activity: '追踪最近的业务变化', weekly: '回顾本周变化，安排下周重点', risk: '集中处理高风险交易', stalled: '识别长时间没有推进的交易', 'deal-lifecycle': '按漏斗阶段查看交易推进', 'action-lifecycle': '按状态查看跟进任务', import: '解析资料并在对话中确认导入', settings: '管理当前工作区和数据导出' };
