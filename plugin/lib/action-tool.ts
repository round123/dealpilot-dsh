...
type Transition = 'active' | 'complete' | 'cancel' | 'block' | 'reopen' | 'schedule';
type ActionStatus = 'active' | 'planned' | 'blocked' | 'done' | 'cancelled';
...
const STATUS_MAP: Record<Transition, ActionStatus> = {
  active: 'active',
  complete: 'done',
  cancel: 'cancelled',
  block: 'blocked',
...
export function registerActionTool(ctx: Record<string, any>, harness: any): void {
...
export async function transitionAction(
  ...
  const allowed = VALID_TRANSITIONS[currentStatus as ActionStatus];

  if (!allowed) {
    throw new Error(
      `未知状态 "${currentStatus}"。已知状态: ${Object.keys(VALID_TRANSITIONS).join(', ')}`,
    );
  }

  if (!allowed.includes(transition)) {
    throw new Error(
      `非法转换: ${currentStatus} → ${transition}。` +
      `允许的转换: ${allowed.join(', ')}`,
    );
  }
  ...
  // Update meta
  const updated: Record<string, any> = { ...meta };
  ...
  updated.generated = { ...((meta.generated as Record<string, any>) || {}), at: now };
...
async function validateActiveActionLimit(
  ...
  if (!dealRef) return; // No deal to check against
