...
function canonicalSelection(content: string, sheetName?: string, rangeValue?: string, includeRaw = true): { content: string; location?: string } | undefined {
  if (!sheetName && !rangeValue) return undefined;
  const document = JSON.parse(content);
  if (document?.schema !== IMPORT_SCHEMA) throw new Error('sheet/range 选择只适用于 dealpilot.import/v1 证据');
  let selectedSheet = sheetName;
  let range = rangeValue || '';
  const sheetRange = /^'?(.+?)'?!(.+)$/u.exec(range);
  if (sheetRange) { selectedSheet = selectedSheet || sheetRange[1].replaceAll("''", "'"); range = sheetRange[2]; }
  const selected = document.sheets.find((item: any) => !selectedSheet || item.name === selectedSheet);
  if (!selected) throw new Error(`找不到指定工作表：${selectedSheet}`);
  let startRow = 1; let endRow = Number.MAX_SAFE_INTEGER; let startColumn = 1; let endColumn = Number.MAX_SAFE_INTEGER;
  if (range) {
    const match = /^([A-Za-z]+)(\d+)(?::([A-Za-z]+)(\d+))?$/u.exec(range);
    if (!match) throw new Error(`范围格式无效：${range}`);
    startColumn = columnNumber(match[1]); startRow = Number(match[2]); endColumn = match[3] ? columnNumber(match[3]) : startColumn; endRow = match[4] ? Number(match[4]) : startRow;
    if (startRow < 1 || endRow < startRow || startColumn < 1 || endColumn < startColumn) throw new Error(`范围格式无效：${range}`);
  }
  const columns = selected.columns.filter((column: any) => column.index + 1 >= startColumn && column.index + 1 <= endColumn);
  const rows = selected.rows.filter((row: any) => row.rowNumber >= startRow && row.rowNumber <= endRow).map((row: any) => ({ rowNumber: row.rowNumber, values: Object.fromEntries(columns.map((column: any) => [column.key, row.values[column.key] ?? ''])), ...(includeRaw ? { raw: Object.fromEntries(columns.map((column: any) => [column.key, row.raw?.[column.key] ?? null])) } : {}), warnings: row.warnings || [] }));
...
async function collectBaseVersions(workspace: string, operations: Operation[]): Promise<Record<string, string>> {
  const versions: Record<string, string> = {};
  for (const operation of operations) {
    const ref = operation?.target?.ref;
    if (typeof ref !== 'string' || !ref) continue;
    const relative = safeRelative(workspace, ref);
    try { versions[relative] = createHash('sha256').update(await fs.readFile(path.join(workspace, relative))).digest('hex'); } catch { /* create targets may not exist yet */ }
  }
  return versions;
}

async function assertBaseVersions(workspace: string, proposal: Proposal): Promise<void> {
  for (const [ref, expected] of Object.entries(proposal.base_versions || {})) {
    let actual = '';
    try { actual = createHash('sha256').update(await fs.readFile(path.join(workspace, ref))).digest('hex'); } catch { /* missing target is a conflict */ }
    if (actual !== expected) throw new Error(`提案目标已发生变化：${ref}。请重新读取并生成提案`);
  }
}

async function refreshVersion(workspace: string, proposal: Proposal, ref: string): Promise<void> {
  try { proposal.base_versions[ref] = createHash('sha256').update(await fs.readFile(path.join(workspace, ref))).digest('hex'); } catch { delete proposal.base_versions[ref]; }
}
...
export function registerAgentMemoryTools(ctx: Record<string, any>, harness: any): void {
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any, exec?: any) {
      ...
      const workspace = resolveWorkspace(ctx.config); validateEvidence(workspace, operations); const now = new Date().toISOString(); const proposal: Proposal = { proposal_id: `prop_${randomUUID()}`, session_id: String(exec?.agent?.id || ''), workspace, operations, base_versions: await collectBaseVersions(workspace, operations), status: 'proposed', applied_operations: [], created_at: now, updated_at: now };
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any, exec?: any) {
      ...
      const results = []; for (let i = 0; i < proposal.operations.length; i++) { if (proposal.applied_operations.includes(i)) continue; try { const result = await applyOperation(workspace, proposal, proposal.operations[i], i); results.push(result); proposal.applied_operations.push(i); await refreshVersion(workspace, proposal, result.ref); } catch (error: any) { proposal.status = 'partially_applied'; proposal.updated_at = new Date().toISOString(); await saveProposal(workspace, proposal); return JSON.stringify({ proposal_id: proposal.proposal_id, status: proposal.status, results, failed_operation: i, error: error.message, retryable: true }); } }
