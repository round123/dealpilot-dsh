...
import { paginateEvidence, validateEvidenceDocument, type EvidenceDocument } from './evidence-contract.js';
import {
  INTERPRETATION_SCHEMA,
  collectInterpretationObservationIds,
  interpretationCoverageAccounting,
  loadInterpretation,
  saveInterpretation,
...
import { applyChangeSet, listTransactions, type ChangeSetInput, type MutationApplyResult, type TransactionRecord } from './mutation-kernel.js';
...
type SessionInfo = { sessionId: string; workspaceId: string };
type ProposalStatus = 'proposed' | 'awaiting_approval' | 'applying' | 'completed' | 'partially_applied' | 'recoverable' | 'failed';

interface ProposalRecord {
  schema: 'dealpilot.proposal/v2';
  proposal_id: string;
  session_id: string;
  workspace_id: string;
  ...
  created_at: string;
  updated_at: string;
  last_result?: MutationApplyResult;
}

const PROPOSAL_SCHEMA = 'dealpilot.proposal/v2' as const;

function proposalReference(id: string): string {
  if (!/^prop_[A-Za-z0-9_-]+$/u.test(id)) throw new Error('提案编号格式无效');
  return `storage/proposals/${id}.json`;
}

function sessionInfo(exec: any): SessionInfo {
 ...
 */
async function resolveRealReference(workspace: string, ref: string): Promise<{ relative: string; filePath: string }> {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') throw new Error(`找不到引用：${relative}`);
    throw error;
  }
...
async function readJsonFile(workspace: string, ref: string): Promise<any> {
  const { relative, filePath } = await resolveRealReference(workspace, ref);
  try { return JSON.parse(await fs.readFile(filePath, 'utf8')); }
  catch (error: any) {
    if (error?.code === 'ENOENT') throw new Error(`找不到引用：${relative}`);
    if (error instanceof SyntaxError) throw new Error(`JSON 内容无效：${relative}`);
    throw error;
  }
}
...
async function readReference(workspace: string, ref: string): Promise<{ ref: string; content: string; metadata?: Record<string, any> }> {
  const { relative, filePath } = await resolveRealReference(workspace, ref);
  let stat;
  try { stat = await fs.stat(filePath); } catch (error: any) {
    if (error?.code === 'ENOENT') throw new Error(`找不到引用：${relative}`);
    throw error;
  }
  ...
  if (relative.endsWith('.json')) {
    let parsed: any;
    try { parsed = JSON.parse(content); } catch { throw new Error(`JSON 内容无效：${relative}`); }
      if (parsed?.schema === EVIDENCE_SCHEMA) validateCanonicalDocument(parsed);
      ...
      if (parsed?.schema === INTERPRETATION_SCHEMA) validateInterpretationDocument(parsed);
    if (parsed?.schema === CHANGE_SET_SCHEMA) validateChangeSetDocument(parsed);
  }
  return { ref: relative, content };
}

function parsedSchema(content: string): string | undefined {
  try { return JSON.parse(content)?.schema; } catch { return undefined; }
}

async function readEvidenceForInterpretation(workspace: string, interpretation: InterpretationDocument, explicitRef?: string): Promise<EvidenceDocument> {
  const job = await readImportJob(workspace, interpretation.import_job_id);
  if (job.canonical_ref !== interpretation.canonical_ref) throw new Error('解释的 canonical_ref 与 Import job 不一致');
  if (job.evidence_digest !== interpretation.evidence_digest) throw new Error('解释的 evidence_digest 与 Import job 不一致');
  const ref = explicitRef ? safeRelative(workspace, explicitRef) : interpretation.canonical_ref;
  if (ref !== job.canonical_ref) throw new Error('显式 evidence_ref 与解释绑定的 canonical_ref 不一致');
  const value = await readJsonFile(workspace, ref);
  if (value?.schema !== EVIDENCE_SCHEMA) throw new Error(`解释必须绑定 ${EVIDENCE_SCHEMA} 证据`);
  validateEvidenceDocument(value);
  if (value.evidence_digest !== job.evidence_digest) throw new Error('Canonical evidence digest 与 Import job 不一致');
  return value as EvidenceDocument;
}

function allObservationIds(evidence: EvidenceDocument): Set<string> {
  const result = new Set<string>();
  for (const sheet of evidence.sheets) {
    for (const column of sheet.columns) result.add(column.header.observation_id);
    for (const row of sheet.rows) for (const cell of row.cells) result.add(cell.observation_id);
  }
  return result;
}

async function resolveInterpretationAndEvidence(workspace: string, interpretation: InterpretationDocument, evidenceRef?: string): Promise<EvidenceDocument> {
...
async function writeExclusiveJson(filePath: string, value: unknown): Promise<void> {
  ...
  try { await fs.writeFile(filePath, `${JSON.stringify(value, null, 2)}\n`, { encoding: 'utf8', flag: 'wx' }); }
  catch (error: any) { if (error?.code === 'EEXIST') throw new Error('提案编号冲突，请重试'); throw error; }
}

async function saveProposal(workspace: string, proposal: ProposalRecord): Promise<void> {
...
async function updateProposal(workspace: string, proposal: ProposalRecord): Promise<void> {
  const destination = await resolveArtifactForWrite(workspace, proposalReference(proposal.proposal_id), 'Proposal');
  ...
  try {
    ...
    try {
      ...
      catch (replaceError) { await fs.rename(backup, destination).catch(() => undefined); throw replaceError; }
...
async function loadProposal(workspace: string, id: string): Promise<ProposalRecord> {
  ...
  try {
    ...
    if (value?.schema !== PROPOSAL_SCHEMA || value.proposal_id !== id) throw new Error('invalid');
    return value as ProposalRecord;
  } catch { throw new Error(`提案不存在：${id}`); }
}

async function referenceResolver(workspace: string, interpretation: InterpretationDocument, evidence: EvidenceDocument): Promise<(ref: string) => Promise<boolean>> {
  ...
  return async (ref: string): Promise<boolean> => {
    if (observations.has(ref) || claims.has(ref)) return true;
    try {
...
function typedChangeSet(value: unknown): ChangeSetInput {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error(`只接受 ${CHANGE_SET_SCHEMA} 对象`);
  const candidate = value as Record<string, unknown>;
  if (candidate.schema !== CHANGE_SET_SCHEMA) throw new Error(`只接受 ${CHANGE_SET_SCHEMA} 的 change_set 字段；请先记录 interpretation 并提供完整 typed 变更集`);
  return candidate as unknown as ChangeSetInput;
}

function selectedOperationIds(changeSet: ChangeSet): string[] { return changeSet.operations.map((operation) => operation.op_id); }

function resultFromTransaction(record: TransactionRecord): MutationApplyResult {
...
async function findPersistedTransaction(workspace: string, changeSet: ChangeSet): Promise<TransactionRecord | undefined> {
  try {
...
async function requestHostApproval(
  ...
  let approval: any;
  try { approval = hostCtx?.get?.('approval') ?? hostCtx?.approval; } catch { approval = hostCtx?.approval; }
  if (!approval || typeof approval.request !== 'function' || !exec?.agent) return 'unavailable';
  const reason = [
...
export function registerAgentMemoryTools(ctx: Record<string, any>, harness: any, hostCtx?: any): void {
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any) {
      ...
      if (schema === EVIDENCE_SCHEMA) {
        const evidence = JSON.parse(result.content) as EvidenceDocument;
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any, exec?: any) {
      const workspace = resolveWorkspace(ctx.config); const { sessionId } = sessionInfo(exec); const value = args.interpretation;
      if (!value || typeof value !== 'object' || value.schema !== INTERPRETATION_SCHEMA) throw new Error(`只接受 ${INTERPRETATION_SCHEMA} 解释文档`);
      if (typeof value.interpretation_id !== 'string' || !value.interpretation_id) throw new Error('interpretation.interpretation_id 必须是非空字符串');
      if (typeof value.import_job_id !== 'string' || !value.import_job_id) throw new Error('interpretation.import_job_id 必须是非空字符串');
      if (typeof value.canonical_ref !== 'string' || !value.canonical_ref) throw new Error('interpretation.canonical_ref 必须是非空字符串');
      const evidence = await resolveInterpretationAndEvidence(workspace, value as InterpretationDocument, args.evidence_ref ? String(args.evidence_ref) : undefined);
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any, exec?: any) {
      const workspace = resolveWorkspace(ctx.config); const { sessionId, workspaceId } = sessionInfo(exec);
      if (args.operations !== undefined) throw new Error(`请先记录 interpretation，再提供完整 ${CHANGE_SET_SCHEMA} 的 change_set 字段`);
      const candidate = typedChangeSet(args.change_set);
      if (typeof candidate.interpretation_id !== 'string' || !candidate.interpretation_id) throw new Error('change_set.interpretation_id 必须是非空字符串');
      ...
      if (candidate.workspace_revision !== currentRevision) throw new Error('Workspace 在形成变更集后发生变化；请重新读取并生成带当前 workspace_revision 的变更集');
      const prepared = { ...candidate, created_at: candidate.created_at || new Date().toISOString() } as ChangeSet;
      const changeSet = { ...prepared, change_set_hash: computeChangeSetHash(prepared) } as ChangeSet;
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any, exec?: any) {
      const workspace = resolveWorkspace(ctx.config); const { sessionId, workspaceId } = sessionInfo(exec); const proposal = await loadProposal(workspace, String(args.proposal_id));
      if (proposal.session_id !== sessionId || proposal.workspace_id !== workspaceId) throw new Error('提案不属于当前 session 或 Workspace');
      if (proposal.status === 'completed') return JSON.stringify({ ok: true, proposal_id: proposal.proposal_id, approval_id: proposal.approval_id, status: proposal.status, transaction_id: proposal.transaction_id, result: proposal.last_result, message: '变更集已经应用；返回之前的事务结果。' });
      if (proposal.status === 'failed') throw new Error('提案已终止，不能重新授权同一变更集；请依据事务记录生成新的 change-set');
      const interpretation = await loadInterpretation(workspace, proposal.interpretation_id); const evidence = await readEvidenceForInterpretation(workspace, interpretation); const ids = allObservationIds(evidence);
      const changeSet = await loadChangeSet(workspace, proposal.change_set_id, { interpretation, evidence, verifyHash: true });
      if (computeChangeSetHash(changeSet) !== proposal.change_set_hash) throw new Error('提案记录与持久化变更集不一致，请重新生成提案');
      validateInterpretationAgainstEvidence(interpretation, evidence); validateChangeSetAgainstInterpretation(changeSet, interpretation, { evidence });
      ...
      if (!approval) {
        const currentRevision = await computeWorkspaceRevision(workspace); if (currentRevision !== changeSet.workspace_revision) throw new Error('Workspace 已变化，原提案失效；请重新读取并生成变更集');
        ...
        proposal.status = 'awaiting_approval'; proposal.approval_id = approval.approval_id; proposal.updated_at = new Date().toISOString();
      ...
      try {
        ...
        result = await applyChangeSet(workspace, changeSet as unknown as ChangeSetInput, { approval, approvalTool: 'dealpilot_apply', sessionId, workspaceId, resolveReference: resolver });
      } catch (error: any) {
      ...
      if (result.status === 'failed' && proposal.approval_id) {
        ...
        if (latest?.status === 'pending') {
          try { revokeApproval(workspace, proposal.approval_id, 'system', 'transaction failed before commit'); } catch { /* keep the transaction result authoritative */ }
        }
      }
      proposal.status = result.status === 'completed' ? 'completed' : result.status === 'recoverable' ? 'recoverable' : result.status === 'partially_applied' ? 'partially_applied' : 'failed'; proposal.transaction_id = result.transaction_id; proposal.last_result = result; proposal.updated_at = new Date().toISOString(); await updateProposal(workspace, proposal);
