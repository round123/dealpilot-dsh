...
const root = (workspace) => path.join(workspace, 'storage', 'indexes');
const indexPath = (workspace) => path.join(root(workspace), 'dealpilot-artifacts.json');
const artifactDir = (workspace) => path.join(workspace, 'sources', 'inbox', '.artifacts');
async function readAll(workspace) {
    try {
...
async function writeAll(workspace, items) {
...
export async function getArtifact(workspace, id) {
    return (await readAll(workspace)).find(item => item.id === id);
}
export async function stageArtifact(workspace, workspaceId, originalName, mediaType, bytes) {
...
export async function readArtifactBytes(workspace, artifact) {
...
export async function updateArtifact(workspace, id, patch) {
    const items = await readAll(workspace);
...
export async function deleteArtifact(workspace, id) {
    const items = await readAll(workspace);
