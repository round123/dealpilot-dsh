...
import { artifactLimits, getArtifact, listArtifacts, readArtifactBytes, stageArtifact, deleteArtifact, type Artifact } from './artifact-store.js';
import { inferFormat, parseXlsx, type ImportFormat } from './import-parser.js';
...
export function registerArtifactTools(ctx: Record<string, any>, harness: any): void {
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    output: { schema: { type: 'object' }, render(_a: any, value: string) { return [{ type: 'text', text: `资料文件：${value}` }]; } },
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: { artifact_id: string; sheet?: string }) {
      const workspace = resolveWorkspace(ctx.config); const artifact = await getArtifact(workspace, args.artifact_id);
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any) {
      const workspace = resolveWorkspace(ctx.config); const artifact = await getArtifact(workspace, args.artifact_id); if (!artifact) throw new Error('Artifact 不存在');
      const format = inferFormat(artifact.originalName, args.format as ImportFormat); const bytes = await readArtifactBytes(workspace, artifact);
      let data: string; let normalized: any;
      if (format === 'xlsx') { normalized = await parseXlsx(bytes, { sheet: args.sheet, headerRow: args.header_row }); data = rowsToCsv(normalized.rows, args.mapping || {}); }
      else data = bytes.toString('utf8');
  ...
  harness.registerTool(ctx, harness.defineTool({
    ...
    async execute(args: any) {
      ...
      const format = inferFormat(artifact.originalName, args.format as ImportFormat); const bytes = await readArtifactBytes(workspace, artifact); let data = bytes.toString('utf8');
      if (format === 'xlsx') { const parsed = await parseXlsx(bytes, { sheet: args.sheet, headerRow: args.header_row }); data = rowsToCsv(parsed.rows, args.mapping || {}); }
      ...
      await updateArtifactStatus(workspace, artifact.id, 'imported'); return result;
    },
  }));
}

function rowsToCsv(rows: Record<string, string>[], mapping: Record<string, string>): string {
  if (!rows.length) return 'title\n';
  const source = Object.keys(rows[0]); const columns = source.map(key => mapping[key] || key); const esc = (value: any) => `"${String(value ?? '').replace(/"/g, '""')}"`;
  return [columns.join(','), ...rows.map(row => source.map(key => esc(row[key])).join(','))].join('\n');
}
async function updateArtifactStatus(workspace: string, id: string, status: any): Promise<void> {
  const store = await import('./artifact-store.js'); await store.updateArtifact(workspace, id, { status });
}

export async function stageUploadedArtifact(workspace: string, workspaceId: string, name: string, mediaType: string, bytes: Buffer): Promise<Artifact> {
