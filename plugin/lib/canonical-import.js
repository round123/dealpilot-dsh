...
export function validateCanonicalDocument(value) {
    if (!value || value.schema !== IMPORT_SCHEMA || !value.source || typeof value.source !== 'object' || !Array.isArray(value.sheets) || !Array.isArray(value.warnings) || !value.provenance)
        throw new Error('canonical JSON schema 无效');
    for (const key of ['sessionId', 'name', 'mediaType', 'sha256'])
        if (typeof value.source[key] !== 'string')
            throw new Error('canonical JSON source 结构无效');
    if (typeof value.provenance.converter !== 'string' || typeof value.provenance.convertedAt !== 'string')
        throw new Error('canonical JSON provenance 结构无效');
    for (const sheet of value.sheets) {
        if (!sheet?.name || !Array.isArray(sheet.columns) || !Array.isArray(sheet.rows))
            throw new Error('canonical JSON 工作表结构无效');
        for (const column of sheet.columns)
            if (typeof column?.key !== 'string' || typeof column?.label !== 'string' || !Number.isInteger(column.index) || column.index < 0)
                throw new Error('canonical JSON 列结构无效');
        for (const row of sheet.rows)
            if (!Number.isInteger(row?.rowNumber) || row.rowNumber < 1 || !row.values || typeof row.values !== 'object' || !row.raw || typeof row.raw !== 'object' || !Array.isArray(row.warnings))
                throw new Error('canonical JSON 行结构无效');
    }
}
