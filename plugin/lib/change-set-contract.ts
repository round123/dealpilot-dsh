...
import {
  CLAIM_STATUSES,
  collectEvidenceObservationIndex,
  type EvidenceObservationInfo,
  validateInterpretationDocument,
  type ClaimStatus,
  type InterpretationDocument,
...
export const CHANGE_SET_SCHEMA = 'dealpilot.change-set/v2' as const;
...
export const ENTITY_OPERATION_MATRIX: Readonly<Record<ChangeEntityType, readonly ChangeOperationKind[]>> =
  Object.freeze(Object.fromEntries(
    Object.entries(ENTITY_ADAPTERS).map(([entity, adapter]) => [entity, Object.freeze([...adapter.operations])]),
  )) as Readonly<Record<ChangeEntityType, readonly ChangeOperationKind[]>>;
export const CHANGE_SET_RISKS = ['low', 'medium', 'high', 'critical', 'unknown'] as const;
export const CHANGE_VALUE_STATUSES = CLAIM_STATUSES;

export type ChangeEntityType = typeof CHANGE_SET_ENTITY_TYPES[number];
export type ChangeOperationKind = typeof CHANGE_SET_OPERATIONS[number];
export type ChangeRisk = typeof CHANGE_SET_RISKS[number];
export type ChangeValueStatus = ClaimStatus;
...
export interface ChangeTarget {
  ...
  extensions?: Record<string, import('./contract-utils.js').JsonValue>;
  [key: string]: unknown;
}
...
export type ChangeEvidenceRef = string | ChangeEvidenceReference;

export interface FieldChange {
  path: string;
  before?: import('./contract-utils.js').JsonValue;
  after?: import('./contract-utils.js').JsonValue;
  value_status: ChangeValueStatus;
  claim_ids: string[];
  evidence_refs: ChangeEvidenceRef[];
  rationale?: string;
  extensions?: Record<string, import('./contract-utils.js').JsonValue>;
}
...
export interface ChangeConflict {
  ...
  description: string;
  resolution?: import('./contract-utils.js').JsonValue;
  extensions?: Record<string, import('./contract-utils.js').JsonValue>;
}
...
export interface ChangeSetValidationOptions {
  interpretation?: InterpretationDocument | unknown;
  evidence?: EvidenceDocument | unknown;
  evidence_document?: EvidenceDocument | unknown;
  evidenceObservationIds?: Iterable<string>;
  evidence_observation_ids?: Iterable<string>;
  ...
  for (const key of Object.keys(value)) {
    if (!allowedSet.has(key)) {
      failContract(
        ...
        `${path}.${key}`,
        `field is not part of change-set/v2; represent it as a typed field_change or explicit note`,
        { field: key },
      );
    }
  }
...
function validateRef(ref: unknown, path: string): string {
  const text = normalizeRef(expectString(ref, path));
  if (/^(?:[A-Za-z]:|\/|\\\\)/u.test(text)) failContract('ABSOLUTE_TARGET_REF', path, 'target ref must be workspace-relative');
...
function pluralEntity(entity: ChangeEntityType): string {
  if (entity === 'evidence') return 'evidence';
  if (entity === 'relationship') return 'relationships';
  return `${entity}s`;
}

function targetTitle(target: ChangeTarget): string | undefined {
  const identityTitle = target.identity && typeof target.identity === 'object'
    ? (target.identity as Record<string, unknown>).title
    : undefined;
  const value = target.title ?? target.label ?? identityTitle;
  return typeof value === 'string' && value.trim() ? value.trim() : undefined;
}

function validateTarget(value: unknown, path: string, entityType: ChangeEntityType, operation: ChangeOperationKind): ChangeTarget {
  if (!isRecord(value)) failContract('TARGET_MUST_BE_OBJECT', path, 'target must be an object; string targets are not accepted');
  const target = value as ChangeTarget;
  ...
  if (identityKeys.length === 0) failContract('TARGET_IDENTITY_REQUIRED', path, 'target needs ref, entity_id, candidate_id, or identity');
  if (target.ref !== undefined) {
    ...
    if (!ref.startsWith(expectedDirectory)) {
      failContract(
        ...
        `${path}.ref`,
        `${entityType} target must resolve under ${expectedDirectory}`,
  ...
  for (const key of ['entity_id', 'candidate_id', 'base_revision', 'expected_revision']) {
    if (target[key] !== undefined) expectString(target[key], `${path}.${key}`);
  }
  if (target.identity !== undefined) {
    const identity = expectRecord(target.identity, `${path}.identity`);
    if (Object.keys(identity).length === 0) failContract('EMPTY_TARGET_IDENTITY', `${path}.identity`, 'identity must contain at least one field');
  ...
  if (operation === 'create' && target.ref === undefined && ['customer', 'contact', 'deal', 'action', 'note'].includes(entityType) && !targetTitle(target)) {
    failContract(
      'TARGET_TITLE_REQUIRED',
      path,
      `${entityType} create requires title, label, or identity.title when ref is omitted`,
    );
  }
  return target;
}

function targetKey(entityType: ChangeEntityType, target: ChangeTarget): string {
  if (target.ref) return `${entityType}:ref:${normalizeRef(String(target.ref))}`;
  if (target.entity_id) return `${entityType}:entity:${target.entity_id}`;
  if (target.candidate_id) return `${entityType}:candidate:${target.candidate_id}`;
...
function validateEvidenceRef(
  ...
  evidenceIndex?: Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }>,
): string {
  ...
  if (!actual && (location !== undefined || sourceId !== undefined || observationHash !== undefined)) {
    failContract(
      ...
      path,
      `metadata for observation ${observationId} cannot be verified without the source evidence document`,
    );
  }
  if (actual) {
...
function validateConflict(value: unknown, path: string): { claimIds: string[]; observationIds: string[] } {
  ...
  if (conflict.resolution !== undefined) expectJsonValue(conflict.resolution, `${path}.resolution`);
...
function validateUnmapped(
  value: unknown,
  path: string,
  evidenceIds?: Set<string>,
  coverageHandling?: Map<string, string>,
): void {
  const item = expectRecord(value, path);
  rejectUnknownKeys(item, ['path', 'observation_ids', 'reason', 'extensions'], path);
  if (item.path !== undefined) expectString(item.path, `${path}.path`);
  ...
  for (let index = 0; index < observationIds.length; index++) {
    if (coverageHandling?.get(observationIds[index]) === 'mapped') {
      failContract(
        'UNMAPPED_COVERAGE_MISMATCH',
        `${path}.observation_ids[${index}]`,
        `observation ${observationIds[index]} is mapped in the interpretation`,
      );
    }
  }
}

function claimIndex(interpretation: InterpretationDocument | unknown): Map<string, any> {
  validateInterpretationDocument(interpretation);
  const result = new Map<string, any>();
  for (const claim of (interpretation as InterpretationDocument).claims) result.set(claim.claim_id, claim);
  return result;
}

function interpretationCitationIndex(
  interpretation: InterpretationDocument,
): Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }> {
  const result = new Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }>();
  for (const claim of interpretation.claims) {
    for (const reference of claim.evidence_refs) {
      ...
      if (existing) {
        for (const key of ['location', 'source_id', 'observation_hash'] as const) {
          const before = existing[key];
          const after = next[key];
          if (before !== undefined && after !== undefined && before !== after) {
            failContract(
              'CONFLICTING_EVIDENCE_METADATA',
              `$.interpretation.claims`,
              `observation ${reference.observation_id} has conflicting ${key} metadata`,
            );
          }
        }
...
function validateFieldChange(
  ...
  operation: ChangeOperationKind,
  claims: Map<string, any> | undefined,
  evidenceIds: Set<string> | undefined,
  evidenceIndex?: Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }>,
  coverageHandling?: Map<string, string>,
): { claimIds: string[]; evidenceIds: string[]; status: ChangeValueStatus } {
  ...
  if (field.before !== undefined) expectJsonValue(field.before, `${path}.before`);
  if (field.after !== undefined) expectJsonValue(field.after, `${path}.after`);
  ...
  for (let index = 0; index < evidenceRefs.length; index++) {
    ...
    if (seenEvidence.has(observationId)) failContract('DUPLICATE_EVIDENCE_REFERENCE', `${path}.evidence_refs[${index}]`, `duplicate observation: ${observationId}`);
    ...
    if (handling && handling !== 'mapped' && !['note', 'evidence'].includes(entityType)) {
      failContract(
        'OBSERVATION_NOT_MAPPED',
        `${path}.evidence_refs[${index}]`,
        `business field cannot be sourced from ${handling} observation ${observationId}`,
        { observation_id: observationId, handling },
      );
    }
  }
  if (['observed', 'inferred', 'hypothesis', 'conflict', 'retracted'].includes(status)) {
    if (claimIds.length === 0) failContract('FACT_CHANGE_WITHOUT_CLAIM', `${path}.claim_ids`, `${status} changes must cite a claim`);
    if (referencedEvidenceIds.length === 0) failContract('FACT_CHANGE_WITHOUT_EVIDENCE', `${path}.evidence_refs`, `${status} changes must cite an observation`);
    ...
    if (!['note', 'evidence'].includes(entityType)) {
      failContract(
        'BUSINESS_CHANGE_WITHOUT_EVIDENCE',
        path,
        'business field changes must cite a claim or observation; use an explicit note for unmapped material',
      );
    }
  }
  if (status === 'unknown' && !['note', 'evidence'].includes(entityType) && field.after !== undefined && field.after !== null) {
    failContract(
      'UNKNOWN_BUSINESS_VALUE',
      `${path}.after`,
      'unknown business values must remain absent/null; retain the uncertainty as a claim or note',
    );
  }
  if (field.rationale !== undefined) expectString(field.rationale, `${path}.rationale`);

  if (claims) {
    const interpretationEvidence = new Map<string, Set<string>>();
    for (const claimId of claimIds) {
      const claim = claims.get(claimId);
      if (!claim) failContract('UNKNOWN_CLAIM_REFERENCE', `${path}.claim_ids`, `unknown claim: ${claimId}`);
      if (claim.status !== status && status !== 'retracted') {
        failContract(
          'CLAIM_STATUS_MISMATCH',
          `${path}.value_status`,
          `field status ${status} does not match claim ${claimId} status ${claim.status}`,
        );
      }
      const refs = new Set<string>(claim.evidence_refs.map((reference: any) => reference.observation_id));
    ...
    for (const observationId of referencedEvidenceIds) {
      if (claimIds.length > 0 && !claimIds.some((claimId) => interpretationEvidence.get(claimId)?.has(observationId))) {
        failContract(
          ...
          `${path}.evidence_refs`,
          `observation ${observationId} is not cited by the selected claims`,
        );
      }
    }
  }
...
function validateOperation(
  ...
  claims: Map<string, any> | undefined,
  evidenceIds: Set<string> | undefined,
  evidenceIndex: Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }> | undefined,
  coverageHandling: Map<string, string> | undefined,
): ChangeOperation {
  ...
  if (operationIds.has(opId)) failContract('DUPLICATE_OPERATION_ID', `${path}.op_id`, `duplicate op_id: ${opId}`);
  ...
  if (!ENTITY_OPERATION_MATRIX[entityType].includes(kind)) {
    failContract(
      'UNSUPPORTED_OPERATION_FOR_ENTITY',
      `${path}.operation`,
      `${entityType} does not support ${kind}`,
      { entity_type: entityType, operation: kind },
    );
  }
  const target = validateTarget(operation.target, `${path}.target`, entityType, kind);
  const key = targetKey(entityType, target);
  if (targetKeys.has(key)) failContract('DUPLICATE_OPERATION_TARGET', `${path}.target`, `target appears more than once: ${key}`);
  targetKeys.add(key);

  const operationRevision = operation.base_revision ?? target.base_revision ?? target.expected_revision;
  if (operationRevision !== undefined) expectString(operationRevision, `${path}.base_revision`);
  ...
  if (targetRevisions) {
    const lookupKeys = [target.ref, target.entity_id, target.candidate_id].filter((item): item is string => typeof item === 'string');
  ...
  for (let index = 0; index < fieldChanges.length; index++) {
    ...
    if (fieldPaths.has(fieldPath)) failContract('DUPLICATE_FIELD_PATH', `${path}.field_changes[${index}].path`, `duplicate field path: ${fieldPath}`);
  ...
  if (claims) {
    for (let index = 0; index < preserve.length; index++) {
      if (!claims.has(preserve[index])) failContract('UNKNOWN_CLAIM_REFERENCE', `${path}.preserve_claim_refs[${index}]`, `unknown claim: ${preserve[index]}`);
    }
  }
  ...
  for (let index = 0; index < conflicts.length; index++) {
    ...
    if (claims) {
      for (let claimIndex = 0; claimIndex < refs.claimIds.length; claimIndex++) {
        if (!claims.has(refs.claimIds[claimIndex])) failContract('UNKNOWN_CLAIM_REFERENCE', `${path}.conflicts[${index}].claim_ids[${claimIndex}]`, `unknown claim: ${refs.claimIds[claimIndex]}`);
      }
    }
    if (evidenceIds) {
      for (let observationIndex = 0; observationIndex < refs.observationIds.length; observationIndex++) {
        if (!evidenceIds.has(refs.observationIds[observationIndex])) failContract('UNKNOWN_OBSERVATION_REFERENCE', `${path}.conflicts[${index}].observation_ids[${observationIndex}]`, `unknown observation: ${refs.observationIds[observationIndex]}`);
      }
    }
    for (let observationIndex = 0; observationIndex < refs.observationIds.length; observationIndex++) {
  ...
  if (operation.idempotency_key !== undefined) expectString(operation.idempotency_key, `${path}.idempotency_key`);
  if (operation.unmapped !== undefined) {
    const unmapped = expectArray(operation.unmapped, `${path}.unmapped`);
    for (let index = 0; index < unmapped.length; index++) validateUnmapped(unmapped[index], `${path}.unmapped[${index}]`, evidenceIds, coverageHandling);
  }
  ...
  if (kind === 'create' && !['note', 'evidence'].includes(entityType)
    && fieldChanges.every((field) => expectStringArray((field as Record<string, unknown>).claim_ids, 'field.claim_ids').length === 0
      && expectArray((field as Record<string, unknown>).evidence_refs, 'field.evidence_refs').length === 0)
  ) {
...
function inspectChangeSet(value: unknown, options: ChangeSetValidationOptions): ChangeSet {
  ...
  let claims: Map<string, any> | undefined;
  let evidenceIds: Set<string> | undefined;
  let evidenceIndex: Map<string, Partial<EvidenceObservationInfo> & { observation_id: string }> | undefined;
  let coverageHandling: Map<string, string> | undefined;
  ...
  if (configuredEvidence !== undefined) {
    ...
    const suppliedEvidenceDigest = digest((configuredEvidence as EvidenceDocument).evidence_digest, '$options.evidence.evidence_digest');
  ...
  if (evidenceObservationIds !== undefined) {
    if (configuredEvidence !== undefined) failContract('AMBIGUOUS_VALIDATION_OPTIONS', '$options', 'provide evidence or evidenceObservationIds, not both');
    evidenceIds = new Set<string>();
    let index = 0;
    for (const item of evidenceObservationIds) {
      const id = expectString(item, `$options.evidenceObservationIds[${index}]`);
      if (evidenceIds.has(id)) failContract('DUPLICATE_OBSERVATION_ID', `$options.evidenceObservationIds[${index}]`, `duplicate observation: ${id}`);
      evidenceIds.add(id);
      index++;
    }
  }
  if (options.interpretation !== undefined) {
    ...
    const interpretation = options.interpretation as InterpretationDocument;
    if (interpretation.interpretation_id !== interpretationId) failContract('INTERPRETATION_ID_MISMATCH', '$.interpretation_id', 'change set references a different interpretation');
    if (interpretation.evidence_digest.toLowerCase() !== evidenceDigest) failContract('EVIDENCE_DIGEST_MISMATCH', '$.evidence_digest', 'change set evidence digest differs from interpretation');
    ...
    if (!evidenceIndex) evidenceIndex = interpretationCitationIndex(interpretation);
    if (!evidenceIds) {
  ...
  if (operations.length === 0) failContract('EMPTY_CHANGE_SET', '$.operations', 'change set must contain at least one operation');
  ...
  if (configuredEvidence !== undefined) {
    const expectedRows = (configuredEvidence as EvidenceDocument).sheets.reduce((total, sheet) => total + sheet.rows.length, 0);
  ...
  if (claims) {
    const coverageAccounting = (options.interpretation as InterpretationDocument).coverage.reduce(
      (result, entry) => {
        result[entry.handling]++;
        return result;
      },
      { mapped: 0, unresolved: 0, ignored: 0 } as Record<string, number>,
    );
    for (const [field, expected] of [
      ...
      const actual = accounting[field] as number;
      if (actual !== expected) {
        failContract(
          'ACCOUNTING_MISMATCH',
          `$.accounting.${field}`,
          `expected ${expected} from interpretation coverage, got ${actual}`,
        );
      }
    }
  }
  if (evidenceIds) {
    ...
    if (accountedObservations !== evidenceIds.size) {
      failContract(
        'ACCOUNTING_MISMATCH',
        '$.accounting',
        `expected ${evidenceIds.size} observations, got ${accountedObservations}`,
      );
    }
  }
  if (document.change_set_hash !== undefined) {
    ...
    if (options.verifyHash ?? options.verify_hash) {
      ...
      if (provided !== actual) failContract('CHANGE_SET_HASH_MISMATCH', '$.change_set_hash', 'change set hash does not match document', { expected: actual, actual: provided });
    }
  }
  ...
  if (document.created_at !== undefined) expectIsoDateTime(document.created_at, '$.created_at');
  return document as unknown as ChangeSet;
}

export function validateChangeSetDocument(value: unknown, options: ChangeSetValidationOptions = {}): asserts value is ChangeSet {
  inspectChangeSet(value, options);
}

export function validateChangeSet(value: unknown, options: ChangeSetValidationOptions = {}): asserts value is ChangeSet {
  inspectChangeSet(value, options);
}

export function parseChangeSetDocument(value: unknown, options: ChangeSetValidationOptions = {}): ChangeSet {
  return inspectChangeSet(value, options);
}

function hashableChangeSet(value: unknown): unknown {
...
export function computeChangeSetHash(value: unknown): string {
...
export function verifyChangeSetHash(value: unknown, expectedHash: string): void {
  const expected = digest(expectedHash, '$expectedHash');
  const actual = computeChangeSetHash(value);
  if (actual !== expected) failContract('CHANGE_SET_HASH_MISMATCH', '$', 'change set hash does not match', { expected, actual });
}

export function operationIdempotencyKey(changeSet: ChangeSet | unknown, operation: ChangeOperation | unknown): string {
...
export function getChangeSetTargetKey(operation: ChangeOperation | unknown): string {
  const item = expectRecord(operation, '$operation');
...
export function buildChangeSetPreview(value: ChangeSet | unknown, options: ChangeSetValidationOptions = {}): ChangeSetPreview {
  const document = inspectChangeSet(value, options);
  const totals: ChangeSetPreview['totals'] = {
...
function changeSetRef(changeSetId: string): string {
  ...
  try {
    ...
    if (computeChangeSetHash(existing) !== computeChangeSetHash(document)) {
      failContract(
        'IMMUTABLE_ARTIFACT',
        '$.change_set_id',
        `change set ${document.change_set_id} already exists with different content`,
      );
    }
    return existing;
  }
...
export async function loadChangeSet(
  ...
  try {
    ...
    if (error?.code === 'ENOENT') throw new Error(`Change set 不存在：${changeSetId}`);
    throw error;
  }
...
export function isChangeSetDocument(value: unknown): value is ChangeSet {
  try {
    inspectChangeSet(value, {});
    return true;
  } catch {
...
export function validateChangeSetAgainstInterpretation(
  value: unknown,
  interpretation: InterpretationDocument | unknown,
  options: Omit<ChangeSetValidationOptions, 'interpretation'> = {},
): asserts value is ChangeSet {
