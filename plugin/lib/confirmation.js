...
export function createConfirmation(operation, payload, message, preview = payload) {
    prune();
...
export function consumeConfirmation(token, operation, payload) {
    prune();
    if (!token)
        throw new Error('此操作需要用户确认。请先向用户展示预览，获得确认后再使用 confirmation_token 重试。');
