...
function consumedApprovalMatchesFeedback(approval, payload, workspacePath, workspaceId, sessionId) {
...
export function createFeedback(input) {
    ...
    const item = { id: `feedback_${randomUUID()}`, kind, title: redact(input.title), body, fingerprint, status: 'draft', createdAt: new Date().toISOString() };
    drafts.set(item.id, item);
    return item;
}
export function registerFeedbackTools(ctx, harness, hostCtx) {
    harness.registerTool(ctx, harness.defineTool({
        ...
        async execute(args) { return createFeedback(args); },
    }));
    harness.registerTool(ctx, harness.defineTool({
        ...
        async execute(args, exec = {}) {
            ...
            if (!approval) {
                let hostApproval;
                try {
                    hostApproval = hostCtx?.get?.('approval') ?? hostCtx?.approval;
                }
                catch {
                    hostApproval = hostCtx?.approval;
                }
