...
interface ImportResult {
  ...
  created: number;
  skipped: number;
  entities: { ref: string; title: string; entity: string }[];
  warnings: string[];
}

// ── Registration ────────────────────────────────────────────────────────────

export function registerImportTool(ctx: Record<string, any>, harness: any): void {
...
导入的客户默认 source_category 为 import，relationship_stage 为 new。`,
    ...
    async execute(args: ImportArgs): Promise<string> {
      ...
      return JSON.stringify(
        await importEntities(workspace, data, format, {
...
export async function importEntities(
  ...
  const existingCustomers = autoDedup ? await loadExistingCustomers(workspace) : new Map<string, string>();
...
function parseCSV(text: string): ImportRecord[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const record: ImportRecord = { title: '', entity: 'customer' };
...
function parseMarkdownTable(text: string): ImportRecord[] {
  const lines = text.trim().split('\n');
  if (lines.length < 2) return [];
...
function parseTextList(text: string): ImportRecord[] {
  const lines = text.trim().split('\n');
  const records: ImportRecord[] = [];
...
async function loadExistingCustomers(workspace: string): Promise<Map<string, string>> {
  const map = new Map<string, string>();
  const docs = await readConceptDir(workspace, 'knowledge/customers');
  for (const doc of docs) {
    const title = (doc.meta.title || '').toLowerCase();
    if (title) map.set(title, doc.ref);
  }
  return map;
}
...
interface DealCreateOptions {
  now: string;
}

async function createCustomerFromImport(
  workspace: string,
  title: string,
  record: ImportRecord,
...
async function createDealFromImport(
  workspace: string,
  title: string,
  record: ImportRecord,
  options: DealCreateOptions,
): Promise<string> {
  const { now } = options;
