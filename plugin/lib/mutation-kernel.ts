...
import {
  CHANGE_SET_SCHEMA as CONTRACT_CHANGE_SET_SCHEMA,
...
export const TRANSACTION_SCHEMA = 'dealpilot.transaction/v2' as const;
...
export type OperationStatus = 'pending' | 'staged' | 'applied' | 'skipped' | 'failed';
...
export type MutationReference = string | EvidenceReference;

export interface FieldChange {
  path: string;
  after: unknown;
  before?: unknown;
  value_status?: 'observed' | 'inferred' | 'hypothesis' | 'unknown' | 'conflict';
  claim_ids?: string[];
  evidence_refs?: MutationReference[];
  rationale?: string;
  extensions?: Record<string, unknown>;
}

export interface MutationOperationInput {
  op_id: string;
  entity_type: MutationEntity;
  operation: MutationOperation;
  target: Record<string, unknown>;
  field_changes: FieldChange[];
  preserve_claim_refs: string[];
...
export interface ChangeAccountingInput {
  source_rows: number;
...
export interface ChangeSetInput {
  ...
  operations: MutationOperationInput[];
  accounting: ChangeAccountingInput;
...
export interface TransactionRecord {
  ...
  actor?: string;
  workspace_revision?: string;
  /** Revision observed after the last journaled operation. */
  ...
  approval_resolutions_hash?: string;
  interpretation_id?: string;
  approval_consumed: boolean;
  status: TransactionStatus;
  created_at: string;
  updated_at: string;
  requested_op_ids: string[];
...
export interface MutationApplyOptions {
  ...
  actor?: string;
  now?: Date;
  /** Optional resolver for claim/evidence references. */
  resolveReference?: (ref: string) => Promise<boolean> | boolean;
}
...
function stagingRoot(workspace: string, transactionId: string): string {
  return path.join(workspace, 'storage', 'staging', transactionId);
}

function journalPath(workspace: string, transactionId: string): string {
  if (!/^tx_[a-f0-9-]{16,}$/iu.test(transactionId)) throw new Error('Invalid transaction id');
...
function referenceIds(values: unknown, name: string): string[] {
...
function operationReferenceIds(operation: MutationOperationInput): string[] {
  const refs: string[] = [];
...
async function hashFile(filePath: string): Promise<ImageRecord> {
  try {
    const content = await fs.readFile(filePath, 'utf8');
    return { exists: true, hash: hashText(content), content };
  } catch (error: any) {
    if (error?.code === 'ENOENT') return { exists: false, hash: '' };
    throw error;
  }
}

async function atomicWrite(filePath: string, content: string, backupPath?: string): Promise<void> {
...
async function currentWorkspaceRevision(workspace: string): Promise<string> {
  return computeWorkspaceRevision(workspace);
}

async function assertWorkspaceRevision(workspace: string, expected: unknown, context: string): Promise<string> {
...
async function writeJournal(workspace: string, journal: TransactionRecord): Promise<void> {
...
function normalizeTransaction(value: unknown): TransactionRecord | undefined {
  if (!value || typeof value !== 'object' || (value as any).schema !== TRANSACTION_SCHEMA) return undefined;
  ...
  return {
    ...(source as unknown as TransactionRecord),
    approval_consumed: source.approval_consumed === true,
    requested_op_ids: Array.isArray(source.requested_op_ids) ? source.requested_op_ids as string[] : [],
    operations: Array.isArray(source.operations) ? source.operations as JournalOperation[] : [],
    completed_ops: Array.isArray(source.completed_ops) ? source.completed_ops as string[] : [],
    skipped_ops: Array.isArray(source.skipped_ops) ? source.skipped_ops as string[] : [],
    failed_ops: Array.isArray(source.failed_ops) ? source.failed_ops as string[] : [],
...
async function loadJournal(workspace: string, transactionId: string): Promise<TransactionRecord | undefined> {
  ...
  if (value === undefined) {
    let previous: any;
    try { previous = await readJson(backup); } catch (error: any) {
  ...
  if (value === undefined && destinationError) throw destinationError;
  return normalizeTransaction(value);
}

async function withWorkspaceLock<T>(workspace: string, callback: () => Promise<T>): Promise<T> {
  ...
  while (true) {
    try {
      ...
      try {
        ...
        try {
          const current = JSON.parse(await fs.readFile(lockPath, 'utf8'));
          if (current?.lease_token === leaseToken) await fs.rm(lockPath, { force: true });
      ...
      if (Date.now() - started >= LOCK_WAIT_MS) throw new Error('Mutation workspace is busy; retry the operation');
...
function validateProtocolChangeSet(changeSet: unknown): asserts changeSet is ChangeSetInput {
...
function fieldChanges(operation: MutationOperationInput): FieldChange[] {
  if (!Array.isArray(operation.field_changes)) throw new Error(`operation ${operation.op_id}: field_changes must be an array`);
  return operation.field_changes.map((change, index) => {
    ...
    if (fieldPath.startsWith('/') || fieldPath.includes('..')) throw new Error(`operation ${operation.op_id}: invalid field path`);
    ...
    return value as unknown as FieldChange;
  });
}

function validateRefs(refs: unknown[], name: string): void {
  refs.forEach((ref, index) => { referenceId(ref, `${name}[${index}]`); });
}

async function validateOperation(
  workspace: string,
  operation: MutationOperationInput,
  seenRefs: Set<string>,
  seenOpIds: Set<string>,
  now: Date,
  resolveReference?: MutationApplyOptions['resolveReference'],
): Promise<{ ref: string }> {
  const opId = requireString(operation.op_id, 'op_id');
  if (seenOpIds.has(opId)) throw new Error(`重复 op_id：${opId}`);
  seenOpIds.add(opId);
  const entity = requireString(operation.entity_type, `operation ${opId}.entity_type`) as MutationEntity;
  const adapter = adapterForEntity(entity);
  const action = requireString(operation.operation, `operation ${opId}.operation`) as MutationOperation;
  if (!adapter.operations.includes(action)) throw new Error(`实体 ${entity} 不支持 ${action} 操作`);
  const target = requireObject(operation.target, `operation ${opId}.target`);
  if (typeof (operation as any).target === 'string') throw new Error(`operation ${opId}.target 不能是字符串`);
  ...
  for (const key of Object.keys(operation)) {
    if (!allowedKeys.has(key)) throw new Error(`operation ${opId}: 不支持字段 ${key}；请通过 field_changes 和 evidence/interpretation 表达内容`);
  }
  fieldChanges(operation);
  if (operation.base_revision !== undefined) requireString(operation.base_revision, `operation ${opId}.base_revision`);
  if (target.base_revision !== undefined) requireString(target.base_revision, `operation ${opId}.target.base_revision`);
  if (target.expected_revision !== undefined) requireString(target.expected_revision, `operation ${opId}.target.expected_revision`);
  ...
  if (action !== 'create' && !target.ref) throw new Error(`operation ${opId}: ${action} 需要 target.ref`);
  ...
  if (seenRefs.has(ref)) throw new Error(`重复目标引用：${ref}`);
  seenRefs.add(ref);
  for (const candidate of operationReferenceIds(operation)) {
    if (resolveReference && !(await resolveReference(candidate))) throw new Error(`找不到证据或声明引用：${candidate}`);
  }
  return { ref };
}
...
export async function validateChangeSet(
  ...
  options: Pick<MutationApplyOptions, 'now' | 'resolveReference'> = {},
): Promise<{ normalized: ChangeSetInput; refs: Array<{ ref: string }> }> {
  validateProtocolChangeSet(changeSet);
  if (!changeSet || typeof changeSet !== 'object') throw new Error('change-set must be an object');
  if (changeSet.schema !== CHANGE_SET_SCHEMA) throw new Error(`只接受 ${CHANGE_SET_SCHEMA} 变更集`);
  const changeSetId = requireString(changeSet.change_set_id, 'change_set_id');
  if (!Array.isArray(changeSet.operations) || !changeSet.operations.length) throw new Error('变更集至少需要一个 operation');
  const seenRefs = new Set<string>();
  const seenOpIds = new Set<string>();
  const refs: Array<{ ref: string }> = [];
  const now = options.now || new Date();
  for (const operation of changeSet.operations) {
    const normalizedOperation = requireObject(operation, 'operation') as unknown as MutationOperationInput;
...
function setNestedValue(target: Record<string, any>, fieldPath: string, value: unknown): void {
  const parts = fieldPath.split('.').filter(Boolean);
  if (!parts.length) throw new Error('field path cannot be empty');
  let cursor = target;
...
function getNestedValue(target: Record<string, any>, fieldPath: string): unknown {
  const parts = fieldPath.split('.').filter(Boolean);
...
function assertFieldBefore(
  operation: MutationOperationInput,
  existing: { meta: Record<string, any>; body: string } | undefined,
): void {
  const changes = fieldChanges(operation);
  for (const change of changes) {
    const record = change as unknown as Record<string, unknown>;
    if (!hasOwn(record, 'before')) continue;
    if (!existing) throw new Error(`operation ${operation.op_id}: before 值要求目标已存在`);
...
function appendContext(operation: MutationOperationInput): string {
  const lines: string[] = [];
...
async function renderAfter(
  ...
  if (entity === 'evidence') {
    ...
    try {
      document = JSON.parse(await fs.readFile(filePath, 'utf8')) as Record<string, any>;
      if (!document || typeof document !== 'object' || Array.isArray(document)) throw new Error(`证据文件必须是 JSON object：${ref}`);
    } catch (error: any) {
      if (error?.code !== 'ENOENT' || operation.operation !== 'create') throw error;
    }
    for (const change of fieldChanges(operation)) {
      ...
      if (change.after !== undefined) setNestedValue(document, change.path, change.after);
    }
    return { content: `${JSON.stringify(document, null, 2)}\n`, metadata: {} };
  }
  let existing: { meta: Record<string, any>; body: string } | undefined;
  try {
  ...
  const metadata = { ...(existing?.meta || {}) } as Record<string, any>;
  const changes = fieldChanges(operation);
  let body = existing?.body || '';
  for (const change of changes) {
    if (operation.entity_type === 'note' && /^(?:body|content)$/iu.test(change.path)) {
      if (change.after !== undefined) body = typeof change.after === 'string' ? change.after : JSON.stringify(change.after, null, 2);
      continue;
    }
    if (change.after !== undefined) setNestedValue(metadata, change.path, change.after);
  }
  ...
  if (evidenceRefs.length) metadata.evidence_refs = Array.from(new Set([...(Array.isArray(metadata.evidence_refs) ? metadata.evidence_refs : []), ...evidenceRefs]));
  if (operation.unmapped?.length) metadata.unmapped_observations = operation.unmapped;
  if (operation.conflicts?.length) metadata.conflicts = operation.conflicts;
  ...
  if (fieldExtensions.length) metadata.field_extensions = fieldExtensions;
  if (operation.operation === 'archive') metadata.status = 'archived';
  ...
  if (operation.operation === 'create' && targetTitle && metadata.title === undefined) metadata.title = targetTitle;
  ...
  if (bodyChanges.length) {
    ...
    if (operation.operation === 'append' || operation.operation === 'link') body = `${body.trimEnd()}\n\n${addition.trim()}\n`;
    else body = `${addition}${appendContext(operation)}`;
...
async function stageOperation(
  ...
  if (operation.operation === 'create' && before.exists) throw new Error(`创建目标已存在：${ref}`);
  if (operation.operation !== 'create' && !before.exists) throw new Error(`目标不存在：${ref}`);
...
async function materializeStaged(workspace: string, operation: JournalOperation): Promise<string> {
  if (!operation.staged_path) throw new Error(`operation ${operation.op_id} 缺少 staging 文件`);
  const staged = await fs.readFile(path.join(workspace, operation.staged_path), 'utf8');
  if (hashText(staged) !== operation.after_hash) throw new Error(`staging 文件校验失败：${operation.op_id}`);
  return staged;
}
...
function eventFor(
  ...
  const refs = operationReferenceIds(operation);
  if (refs.length) event.reference_ids = Array.from(new Set(refs));
  ...
  if (operation.entity_type === 'customer') event.customer_ref = ref;
  if (operation.entity_type === 'deal') event.deal_ref = ref;
  if (operation.entity_type === 'action') event.action_ref = ref;
  return event;
}

async function appendEventOnce(workspace: string, event: BusinessEvent): Promise<void> {
  const eventPath = path.join(workspace, 'knowledge', 'events', 'business-events.jsonl');
  let existing = '';
  try { existing = await fs.readFile(eventPath, 'utf8'); } catch (error: any) { if (error?.code !== 'ENOENT') throw error; }
...
async function commitOperation(workspace: string, item: JournalOperation): Promise<Record<string, unknown>> {
  ...
  if (!after.exists || after.hash !== item.after_hash) throw new Error(`提交后目标校验失败：${item.ref}`);
  if (item.backup_path) {
    try { await fs.rm(path.join(workspace, item.backup_path), { force: true }); } catch {}
  }
 ...
 */
async function repairAppliedOperationDerivatives(workspace: string, item: JournalOperation): Promise<void> {
  const filePath = path.join(workspace, item.ref);
  const image = await hashFile(filePath);
  if (!image.exists || image.hash !== item.after_hash) {
    throw new Error(`已应用操作的权威文件不匹配，拒绝重放：${item.ref}`);
  }
  if (!item.event?.event_id) throw new Error(`已应用操作缺少审计事件：${item.op_id}`);
  await appendEventOnce(workspace, item.event);
  if (isBusinessEntity(item.entity_type)) {
    const parsed = await readYamlFrontmatter(filePath);
    const indexPath = path.join(workspace, 'storage', 'indexes', `${item.entity_type}.json`);
    const index = await readJson(indexPath);
    const matches = Array.isArray(index) ? index.filter((entry: any) => entry?.ref === item.ref) : [];
...
function invariants(journal: TransactionRecord): Record<string, boolean> {
  ...
  const statusMemberships = (opId: string): number => Number(completedSet.has(opId)) + Number(skippedSet.has(opId)) + Number(failedSet.has(opId));
  const operationStatusesConsistent = requestedIds.length === requestedSet.size
    ...
    && journal.operations.every(item => {
      ...
      if (item.status === 'applied') return completedSet.has(item.op_id) && !skippedSet.has(item.op_id) && !failedSet.has(item.op_id);
      if (item.status === 'skipped') return skippedSet.has(item.op_id) && !completedSet.has(item.op_id) && !failedSet.has(item.op_id);
      if (item.status === 'failed') return failedSet.has(item.op_id) && !completedSet.has(item.op_id) && !skippedSet.has(item.op_id);
      return membership === 0;
    });
  return {
...
function resultFromJournal(journal: TransactionRecord): MutationApplyResult {
...
async function eventIdsInWorkspace(workspace: string): Promise<Set<string>> {
  const result = new Set<string>();
  const eventPath = path.join(workspace, 'knowledge', 'events', 'business-events.jsonl');
  let content = '';
  try { content = await fs.readFile(eventPath, 'utf8'); } catch (error: any) { if (error?.code !== 'ENOENT') throw error; }
  for (const line of content.split(/\r?\n/u)) {
    if (!line.trim()) continue;
    let value: any;
    try { value = JSON.parse(line); } catch { continue; }
    if (typeof value?.event_id === 'string') result.add(value.event_id);
  }
  return result;
}

async function verifyWriteInvariants(workspace: string, journal: TransactionRecord): Promise<Record<string, boolean>> {
  ...
  for (const item of journal.operations.filter(operation => operation.status === 'applied')) {
    if (seenRefs.has(item.ref)) checks.all_refs_are_unique = false;
    seenRefs.add(item.ref);
    const image = await hashFile(path.join(workspace, item.ref));
    if (!image.exists || image.hash !== item.after_hash) authoritativeFilesMatch = false;
    if (!eventIds.has(item.event_id)) businessEventsRecorded = false;
    if (isBusinessEntity(item.entity_type)) {
      ...
      if (matches.length !== 1) {
        ...
        try {
          ...
          const entry = matches[0] as Record<string, any>;
...
async function findExistingTransaction(workspace: string, changeSetId: string, changeSetHash: string): Promise<TransactionRecord | undefined> {
  let names: string[] = [];
  try { names = await fs.readdir(transactionRoot(workspace)); } catch (error: any) { if (error?.code !== 'ENOENT') throw error; }
  ...
  for (const name of candidates) {
    ...
    if (journal?.change_set_id === changeSetId) {
      if (journal.change_set_hash !== changeSetHash) throw new Error(`change_set_id 已对应不同内容：${changeSetId}`);
      return journal;
    }
  }
  return undefined;
...
async function assertResumeContext(
  workspace: string,
  journal: TransactionRecord,
  options: MutationApplyOptions,
): Promise<void> {
  if (workspaceFingerprint(workspace) !== workspaceFingerprint(journal.workspace)) throw new Error('事务不属于当前 Workspace');
  ...
  if (!journal.workspace_revision) return;
  ...
  for (const item of journal.operations) {
    ...
    const current = await hashFile(path.join(workspace, item.ref));
    if (current.hash === item.after_hash) hasUnjournaledAfterImage = true;
...
async function revalidateCompletedTransaction(
  workspace: string,
  journal: TransactionRecord,
...
function approvalBinding(
  ...
  changeSet: ChangeSetInput,
  options: MutationApplyOptions,
): Parameters<typeof validateApproval>[1] {
  ...
  if (!options.sessionId) throw new Error('应用变更集需要 sessionId');
  return {
...
async function verifyApproval(
  ...
  if (options.approval) {
    ...
    if (approval.status !== 'pending') throw new Error('批准记录已失效，请重新审阅');
    return validateApprovalRecord(approval, binding);
  }
  if (!options.approvalToken) throw new Error('缺少持久化用户批准记录');
  return validateApproval(options.approvalToken, binding);
}

function consumeVerifiedApproval(
  workspace: string,
  changeSet: ChangeSetInput,
  options: MutationApplyOptions,
): ApprovalRecord {
  const binding = approvalBinding(workspace, changeSet, options);
  if (options.approval) return consumeApprovalRecord(options.approval, binding);
  if (!options.approvalToken) throw new Error('缺少持久化用户批准记录');
  return consumeApproval(options.approvalToken, binding);
}

function sortedIds(values: unknown): string[] | undefined {
  if (!Array.isArray(values) || values.some(value => typeof value !== 'string' || !value.trim())) return undefined;
 ...
 */
function approvalMatchesJournal(
  workspace: string,
  journal: TransactionRecord,
  stored: ApprovalRecord | undefined,
): boolean {
  try {
    if (!stored || stored.status !== 'consumed') return false;
    ...
    if (stored.approval_id !== journal.approval_id
      ...
      || journal.approval_resolutions_hash !== hashText(canonicalJson(stored.resolutions || {}))) return false;

    const payload = stored.payload;
    validateChangeSetDocument(payload, { verifyHash: true });
    const document = payload as unknown as ChangeSetInput;
    if (hashText(canonicalJson(payload)) !== stored.payload_hash
      ...
      || canonicalJson(sortedIds(document.operations.map(operation => operation.op_id)) || [])
        !== canonicalJson(requestedIds)) return false;
    return true;
  } catch {
    return false;
  }
...
function copyApprovalBindingToJournal(journal: TransactionRecord, approval: ApprovalRecord): void {
...
async function ensureTransactionApproval(
  workspace: string,
  changeSet: ChangeSetInput,
  journal: TransactionRecord,
  options: MutationApplyOptions,
  ...
  if (journal.approval_consumed === true) {
    if (!approvalMatchesJournal(workspace, journal, stored)) throw new Error('事务批准绑定无法验证，请重新审阅');
    return;
  }
  ...
  if (journal.approval_id && consumed.approval_id !== journal.approval_id) throw new Error('批准记录与事务不一致');
  if (journal.approval_token_hash && consumed.token_hash !== journal.approval_token_hash) throw new Error('批准令牌与事务不一致');
  copyApprovalBindingToJournal(journal, consumed);
  journal.updated_at = new Date().toISOString();
  await writeJournal(workspace, journal);
}

async function recoverApprovalFlag(workspace: string, journal: TransactionRecord): Promise<boolean> {
  const stored = journal.approval_id ? readApproval(workspace, journal.approval_id) : undefined;
  if (!approvalMatchesJournal(workspace, journal, stored)) return false;
  if (journal.approval_consumed !== true) {
    copyApprovalBindingToJournal(journal, stored!);
    journal.updated_at = new Date().toISOString();
    await writeJournal(workspace, journal);
...
export async function applyChangeSet(
  ...
  return withWorkspaceLock(workspace, async () => {
    ...
    if (existing) await assertResumeContext(workspace, existing, options);
...
export async function recoverTransactions(workspace: string, options: MutationApplyOptions = {}): Promise<MutationApplyResult[]> {
  const root = transactionRoot(workspace);
  let names: string[] = [];
  try { names = await fs.readdir(root); } catch (error: any) { if (error?.code !== 'ENOENT') throw error; }
  ...
  await withWorkspaceLock(workspace, async () => {
    ...
    for (const name of candidates) {
      const journal = await loadJournal(workspace, name.slice(0, -5));
      if (!journal || !['applying', 'recoverable', 'partially_applied'].includes(journal.status)) continue;
      try {
        ...
        const recoveryOptions: MutationApplyOptions = {
          ...options,
...
export async function readTransaction(workspace: string, transactionId: string): Promise<TransactionRecord | undefined> {
  return loadJournal(workspace, transactionId);
}

export async function listTransactions(workspace: string): Promise<TransactionRecord[]> {
  let names: string[] = [];
  try { names = await fs.readdir(transactionRoot(workspace)); } catch (error: any) { if (error?.code !== 'ENOENT') throw error; }
