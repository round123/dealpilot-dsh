...
export type WhatsAppAction = 'pull_messages' | 'analyze' | 'prepare_draft';

export interface WhatsAppMessage {
  sender: string;   // "me" or contact name
  text: string;     // message content
  timestamp: string; // message time
}

export interface WhatsAppToolArgs {
...
export type WhatsAppResult = PullMessagesResult | AnalyzeResult | DraftResult;

// ── Registration ────────────────────────────────────────────────────────────

export function registerWhatsappTool(ctx: Record<string, any>, harness: any) {
...
如果没有扩展，Agent 可以直接将消息内容作为 messages 参数传入。`,
    ...
    async execute(args: WhatsAppToolArgs): Promise<string> {
      ...
      if (action === 'analyze') {
        ...
        return JSON.stringify(
          await analyzeMessages(workspace, conversation_key, messages, customer_ref, deal_ref),
        );
      }
      ...
      if (action === 'prepare_draft') {
        ...
        return JSON.stringify(
          prepareDraft(conversation_key, messages, customer_ref, deal_ref),
        );
      }
...
async function analyzeMessages(
  ...
  if (!customer && conversationKey) {
    ...
    const found = docs.find(d =>
      (d.meta.title || '').toLowerCase().includes(keyLower),
    );
    if (found) {
      customer = { ref: found.ref, title: found.meta.title };
    }
...
function extractFacts(messages: WhatsAppMessage[]): ExtractedFact[] {
...
function analyzeSentiment(messages: WhatsAppMessage[]): 'positive' | 'negative' | 'neutral' {
  ...
  for (const msg of messages) {
    if (msg.sender === 'me') continue;
    if (positiveWords.test(msg.text)) positive++;
    if (negativeWords.test(msg.text)) negative++;
  }
...
function detectLanguage(messages: WhatsAppMessage[]): 'zh' | 'en' {
  let zhCount = 0;
  let enCount = 0;
  for (const msg of messages) {
...
function generateChineseDraft(facts: ExtractedFact[], sentiment: string): string {
...
function generateEnglishDraft(facts: ExtractedFact[], sentiment: string): string {
  const hasPrice = facts.some(f => f.type === 'price');
  const hasRequirement = facts.some(f => f.type === 'requirement');
...
function generateSuggestedActions(facts: ExtractedFact[], sentiment: string): SuggestedAction[] {
