...
const BODY_SECTIONS: Record<EntityType, string[]> = {
...
export function registerWriteTool(ctx: Record<string, any>, harness: any): void {
...
- 归档操作是高影响操作`,
    ...
    output: {
      ...
      presentationMeta(args: WriteToolArgs, value: any) {
        return writePresentation(args, value);
      },
    },
    async execute(args: WriteToolArgs): Promise<string> {
      ...
      if (operation === 'merge') {
        if (entity !== 'customer') throw new Error('目前仅支持合并 customer；请先分别确认其他对象的更新或归档');
        if (!safeRef || !safeSourceRef) throw new Error('merge 操作需要 ref 和 source_ref 参数');
        if (safeRef === safeSourceRef) throw new Error('merge 的目标和来源不能是同一个对象');
      }
      ...
      if (!confirmation_token) {
        const preview = operation === 'merge'
          ...
          : operation === 'update' && safeRef
            ...
            : undefined;
        const hasConflict = Boolean(preview && Array.isArray(preview.changes) && preview.changes.some((change: any) => change.conflict));
        return JSON.stringify(createConfirmation(
          ...
          operation === 'merge'
            ...
            : hasConflict
              ...
              : '这是一个会修改销售工作区的操作。请向用户展示目标、字段和影响，获得明确确认后再重试。',
          preview || confirmationPayload,
        ));
      }
...
async function buildMergePreview(workspace: string, targetRef: string, sourceRef: string): Promise<Record<string, any>> {
  let target: { meta: Record<string, any>; body: string };
  let source: { meta: Record<string, any>; body: string };
  try {
...
async function buildUpdatePreview(
  ...
  let existing: { meta: Record<string, any>; body: string };
  try {
...
async function mergeEntity(
  ...
  sourceRef: string,
  now: string,
): Promise<WriteResult & { mergedRef: string; conflicts: string[] }> {
  if (entity !== 'customer') throw new Error('目前仅支持合并 customer');
  ...
  if (target.meta.status === 'archived') throw new Error('合并目标客户已归档');
  if (source.meta.status === 'archived') throw new Error('合并来源客户已归档');

  const conflictKeys = ['market', 'relationship_stage', 'icp_fit', 'priority', 'source_category', 'source_label']
    .filter((key) => target.meta[key] !== undefined && source.meta[key] !== undefined && JSON.stringify(target.meta[key]) !== JSON.stringify(source.meta[key]));
  const mergedMeta: Record<string, any> = { ...source.meta, ...target.meta };
...
async function rewireCustomerReferences(workspace: string, sourceRef: string, targetRef: string, now: string): Promise<number> {
  let count = 0;
  const deals = await readConceptDir(workspace, 'knowledge/deals');
  for (const doc of deals) {
    if (doc.meta.customer !== sourceRef) continue;
...
function mergeBodies(targetBody: string, sourceBody: string, sourceRef: string): string {
  const target = targetBody.trim();
  const source = sourceBody.trim();
  if (!source) return target;
  if (!target) return `# Merged source\n\nSource: ${sourceRef}\n\n${source}`;
  return `${target}\n\n## Merged source (${sourceRef})\n\n${source}\n`;
}

/** Convert human-entered relationship names to verified workspace refs. */
async function normalizeRelationshipFields(
  workspace: string,
  entity: EntityType,
  fields: Record<string, any>,
): Promise<Record<string, any>> {
  ...
  if (!relation || typeof normalized[relation] !== 'string' || !normalized[relation].trim()) return normalized;
  ...
  if (value.startsWith('knowledge/')) {
    ...
    if (!docs.some(doc => doc.ref === safe)) throw new Error(`找不到关联对象：${value}`);
  ...
  if (matches.length > 1) throw new Error(`关联对象名称不唯一：${value}。请使用对象 ref。`);
...
async function createEntity(
  ...
  for (const [key, value] of Object.entries(fields)) {
    if (['title', 'status', 'body', 'content', 'generated', 'ref', ...bodyKeys].includes(key)) continue;
  ...
  if (entity === 'deal') {
    event.deal_ref = ref;
    if (fields.customer) event.customer_ref = fields.customer;
  }
  if (entity === 'action') {
    event.action_ref = ref;
    if (fields.deal) event.deal_ref = fields.deal;
  }
  await appendBusinessEvent(workspace, event);

  // Update index
  const indexEntry: IndexEntry = { ref, ...meta, updated_at: now };
  if (entity === 'deal') {
...
async function updateEntity(
  ...
  // Update index
  const indexEntry: IndexEntry = { ref, ...updated, updated_at: now };
  if (entity === 'deal') {
...
function buildBody(entity: EntityType, fields: Record<string, any>): string {
...
function isBodySection(entity: EntityType, key: string): boolean {
  const sections = BODY_SECTIONS[entity] || [];
  return sections.some(s => s.toLowerCase().replace(/\s+/g, '_') === key);
}

function updateBodySection(body: string, key: string, value: any): string {
...
function detectChangeEventType(
  entity: EntityType,
...
async function validateActiveActionLimit(
  workspace: string,
  dealRef: string,
  ...
  for (const doc of actions) {
    if (doc.meta.deal !== dealRef) continue;
    if (excludeRef && doc.ref === excludeRef) continue;
    if (doc.meta.status === 'active') {
      throw new Error(
        `每个 Deal 最多一个 active Action。Deal ${dealRef} 已有 active Action: ${doc.ref}`,
      );
    }
  }
}
